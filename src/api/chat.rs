...
/// Uses async_channel for smol/GPUI compatibility
pub struct ChatConnection {
    /// Sender for outgoing messages (async_channel for smol compatibility)
    pub sender: async_channel::Sender<String>,
    /// Receiver for incoming messages (async_channel for smol compatibility)
    pub receiver: async_channel::Receiver<ChatEvent>,
}
...
/// This function should be called from within a tokio context (e.g., wrapped in Compat)
pub async fn connect_chat(
    ...
    // Create async_channel for external communication (smol-compatible)
    let (tx_out_ext, rx_out_ext) = async_channel::bounded::<String>(100);
    let (tx_in_ext, rx_in_ext) = async_channel::bounded::<ChatEvent>(100);

    // Create tokio channels for internal communication
    let (tx_out_int, mut rx_out_int) = tokio_mpsc::channel::<String>(100);
    ...
    if let (Some(token), Some(user)) = (&access_token, &username) {
        ...
        write
            .send(Message::Text(
                format!("NICK {}", user.to_lowercase()).into(),
            ))
            .await?;
    ...
    let _ = tx_in_ext.send(ChatEvent::Connected).await;

    let channel_for_read = channel.clone();
    let channel_for_write = channel.clone();

    // Spawn task to handle incoming messages
    let tx_in_read = tx_in_ext.clone();
    ...
    // Spawn task to bridge external async_channel to internal tokio channel
    let tx_out_bridge = tx_out_int.clone();
    tokio::spawn(async move {
        while let Ok(msg) = rx_out_ext.recv().await {
            if tx_out_bridge.send(msg).await.is_err() {
                break;
            }
        }
        info!("[Chat] Outgoing message bridge ended");
    });
