...
**Your own messages are echoed locally.** Twitch IRC never sends your `PRIVMSG` back to you — it relays it to the other viewers and answers you with a `USERSTATE`. `chat.rs` queues each sent text in `pending_echo` and pairs it with the next `USERSTATE` to build the `chat-message` event the frontend would otherwise never see. A ` NOTICE ` (message refused: slow mode, subs only, ban, unverified account) drops the queued text, so a rejected message is not echoed — the notice itself is emitted as `chat-notice` and `useChat` appends it to the message list as a `system: true` entry, rendered by `ChatNoticeView`. Before this, `chat-notice` had no listener at all and a refused message failed silently. The echo has no IRC id (a `self-N` one is generated) and no emote ranges, so Twitch emotes in your own message render through the name-based lookup in `allEmotes`.

**Chat is one IRC connection per channel.** `AppState.chats` is a `HashMap<String, ChatEntry>`, and `send_chat_message` takes the channel so a message always goes where you typed it. Each `useChat` instance registers a subscriber id shaped `<window label>:<uuid>`; `connect_to_chat` reuses a live connection and only adds the id, `disconnect_from_chat` closes the connection when the last subscriber leaves. Subscriber **ids, not a refcount**: releasing twice (React cleanup plus the window-destroy sweep) must not close a connection another window still uses. A closed pop-out never runs its React cleanup, so `on_window_event(Destroyed)` drops every subscriber matching the window's label prefix.

This replaced a single-slot design where a pop-out stole the one sender: messages typed in the main window were then delivered to the pop-out's channel, and the echo — carrying the wrong channel — was filtered out, so nothing appeared. The old `chat_handle` field was never assigned, so the abort meant to cancel the previous read loop was dead code and stale sockets lived until Twitch reset them.

`connect_to_chat` rebuilds an entry (keeping its subscribers) in two cases: `read_task.is_finished()`, meaning the socket died, and a change of login state. Without the first, the frontend's reconnect after `chat-disconnected` would find the dead entry and return `Ok` without reconnecting. The second matters because an anonymous connection can read but not send — Twitch drops a `justinfan` PRIVMSG with no notice at all — so a session that connected before login would silently swallow every message until the channel changed. `useChat` keys its connect guard on `${channel}:${isLoggedIn}` for the same reason.
